...
const UserSchema = z.object({
    ...
    password: z.string().min(6),  // Password is required with a minimum length of 6
    confirmPassword: z.string().min(6),  // Password is required with a minimum length of 6
});
import styled from 'styled-components'
import { toast } from 'sonner'
import { useMutation } from '@apollo/client'
import { CREATE_USER_MUTAION } from '../../queries/mutations/user.mutation'
...
const Signup = () => {
    ...
    const [createUser, { data: userData, loading: creatingUser, error }] = useMutation(CREATE_USER_MUTAION, {
        onCompleted: async () => {
    ...
    const submitHandle: SubmitHandler<formData> = async (data) => {
        try {
            ...
            const user = await createUser({ variables: { input: { password, username, email } } })
        ...
        <Container >
            <form className="font-customFont w-screen h-screen flex items-center justify-end bg-no-repeat bg-center bg-cover bg-local overflow-hidden" onSubmit={handleSubmit(submitHandle)}>
                ...
                >
                    ...
                    <motion.input type="email" {...register("email")} placeholder='Your Email' className={`border-b-4 pb-1  text-md outline-none focus:border-secondary-color transition-all ease-in-out duration-700 ${errors.email ? "mb-0 border-red-300" : "mb-6"}`}
                        initial={{
                            opacity: 0
                        }}
                        animate={{
                    ...
                    />
                    {errors.email && <span className='mb-5 text-xs text-red-500'>*{errors.email.message}</span>}
                    ...
                    <motion.input type="password" {...register("confirmPassword")} placeholder='Confirm Password' className={`border-b-4 pb-1  text-md outline-none focus:border-secondary-color transition-all ease-in-out duration-700 ${errors.confirmPassword ? "mb-0 border-red-300" : "mb-6"}`}
                        initial={{
                            opacity: 0
                        }}
                        animate={{
                    ...
                    />
                    {errors.confirmPassword && <span className='mb-5 text-xs text-red-500'>*{errors.confirmPassword.message}</span>}
                    ...
                    >
                        ...
                        <span className="relative text-white group-hover:text-custom-purple-dark flex items-center justify-center">
                            {isSubmitting || creatingUser ? <img className='w-11' src={complete} alt="" /> : "Create Account"}</span>
                    </motion.button>
                    <motion.span
                        initial={{
                            opacity: 0
                    ...
                    ><Link to={'/login'} className="text-blue-600 hover:text-blue-500 hover:underline text-sm mt-1 ms-2">Already have an Account?</Link></motion.span>
                </motion.div>
            </form>
        </Container>
    )
